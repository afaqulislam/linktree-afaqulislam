...
const SparkleLinkParticleGroup = () => {
  ...
  >(() => {
    ...
    const random = (seed: number, min: number, max: number) => {
      return Math.floor(
        seededRandom(seed) * (max - min + 1) + min
      )
    }

    return Array.from({ length: 20 }).map((_, i) => {
      const delay = random(i + 1, 1, 10)
      const alpha = random(i + 2, 40, 90) / 100

      const originXValue = random(i + 3, 300, 800)
      const originYValue = random(i + 4, 300, 800)

      const originX =
        seededRandom(i + 5) > 0.5
          ? originXValue * -1
          : originXValue

      const originY =
        seededRandom(i + 6) > 0.5
          ? originYValue * -1
          : originYValue
      ...
      return {
        ...
        style: {
          '--x': `${random(i + 7, 20, 80)}`,
          '--y': `${random(i + 8, 20, 80)}`,
          '--duration': `${random(i + 9, 6, 20)}`,
          '--delay': `${delay}`,
          '--alpha': `${alpha}`,
          '--origin-x': `${originX}%`,
          '--origin-y': `${originY}%`,
          '--size': `${random(i + 10, 40, 90) / 100}`,
